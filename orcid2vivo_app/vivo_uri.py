...
def to_hash_identifier(prefix, parts):
    ...
    """
    hash_parts = hashlib.md5("".join([str(part) for part in parts if part]).encode("utf-8"))
...
class HashIdentifierStrategy():
    ...
    def _attrs_to_hash(attrs):
        ...
        hash_parts = hashlib.md5("".join([str(part) for part in sorted_attrs.values() if part]).encode("utf-8"))
