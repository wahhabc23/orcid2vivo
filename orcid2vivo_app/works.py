...
class WorksCrosswalk:
    ...
    def crosswalk(self, orcid_profile, person_uri, graph):
        ...
        for work_group in work_groups:
            for work in work_group.get("work-summary") or []:
                path = work.get("path")
                if path:
                    self.crosswalk_work(self._fetch_work(path), person_uri, person_surname, graph)
    ...
    def crosswalk_work(self, work, person_uri, person_surname, graph):
        ...
        if work_type in work_type_map:
            ...
            for identifier_type, identifier in external_identifiers.items():
                identifier_url = None
    ...
    def _use_url(url):
        ...
        for (identifier_predicate, url_template) in identifier_map.values():
            if url_template:
